...
### ros map
rosbagファイルとpbstreamファイルからros mapファイルを作成します。<br>
ros mapはros navigationでLocalizationとPath Plannerを実行する時に使います。<br>
引数は`myroom.bag`だけですが、実行時に`myroom.bag.pbstream`ファイルも参照します。<br>
`myroom.bag_map.pgm`と`myroom_bag_map.yaml`が作成されます。<br>
```
...
### 06.offline_slam.sh
Cartographer-ROSのLocalizationで利用するpbstream形式のマップを作成します。<br>
...
```
引数にbagファイルを指定します。<br>
pbstreamのファイル名は`bagファイル名`.pbstreamとしています。<br>
```
# usage:
...
ros map作成
```
roslaunch cartographer_ros offline_ydlidar_2d_assets_writer_ros_map.launch bag_filenames:=${INPUT_BAG} pose_graph_filename:=${INPUT_BAG}.pbstream
```