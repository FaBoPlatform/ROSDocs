...
turtlebot3 navigationでは、デフォルトの`map_server`を`cartographer_ros`に変更することで、navigationにも利用できます。この場合、IMU無しで動作可能になります。<br>
...
### ros map
ros mapはturtlebot3 navigationでLocalizationとPath Plannerを実行する時に使います。この場合、別途IMUが必要になります。<br>
