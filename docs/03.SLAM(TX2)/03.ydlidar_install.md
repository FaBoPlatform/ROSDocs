...
turtlebot3のnavigation走行には、`/map`フレームに`/base_footprint`が必要になるので、`frame_id` `child_frame_id`は`/map`と`/base_footprint`を指定します。<br>
turtlebot3のnavigation走行には、`/odom`フレームに`/base_footprint`が必要になるので、`frame_id` `child_frame_id`は`/odom`フレームに`/base_footprint`を指定します。<br>
...
+++ /home/ubuntu/catkin_ws/src/ydlidar/launch/lidar.launch	2019-04-25 17:06:56.181032218 +0900
@@ -1,20 +1,22 @@
 <launch>
...
+  <node pkg="tf" type="static_transform_publisher" name="odom_tf"
